...
var JFP_CreateOperatorError = error.JFP_CreateOperatorError;
var JFP_ParseOperandError = error.JFP_ParseOperandError;
    ...
    switch (this.operandsType) {
        case "array":
            if (!Array.isArray(operand)) {
                throw new JFP_ParseOperandError(this, operand, "Array expected");
            }
            if (this.operandsCountMin && operand.length < this.operandsCountMin) {
                throw new JFP_ParseOperandError(this, operand, "Not enough operands");
            }
            if (this.operandsCountMax && operand.length > this.operandsCountMax) {
                throw new JFP_ParseOperandError(this, operand, "Too many operands");
            }

            var parts = [];

            for (var i = 0; i < operand.length; i++) {
                parts[i] = parse(operand[i], operators, this);
            }
        ...
        case "regex":
            ...
            if (typeof operand == "string") {
                ...
                pattern = parse(operand, operators, this);
            }
        ...
        case "operator":    // $not
            ...
            operand = parse(operand, operators, this);
            if (operand.operator.rightHanded && !this.rightHanded) {
                operand = {$eq: operand};
            }

            return parse(operand, operators, this, true);

        case "context":
            if (typeof operand != "string") {
                throw new JFP_ParseOperandError(this, operand, "String expected");
            }

            return operand;

        default:
            return parse(operand, operators, this);
    }
};
...
JsonFilterOperator.create = function (data) {
    if (!data.name || !data.name.match(REGEX_NAME)) {
        throw new JFP_CreateOperatorError(data, "Incorrect name");
    }

    if (OPERAND_TYPES.indexOf(data.operandsType) == -1) {
        throw new JFP_CreateOperatorError(data, "Unknown operandsType");
    }
